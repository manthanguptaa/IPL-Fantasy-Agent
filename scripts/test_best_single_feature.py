#!/usr/bin/env python3
"""Test the best single feature (won_toss) and save the improved model."""
...
from scripts.add_features_and_test import add_toss_advantage_feature


def test_won_toss_feature(
    features_path: str,
    n_matches: int = 50,
    save_model: bool = True,
):
    """Test won_toss feature and save improved model."""
    ...
    print(f"\n{'='*60}")
    print("Testing BEST single feature: won_toss")
    print(f"{'='*60}")
    ...
    # Extended features with just won_toss
    extended_features = OPTIMAL_FEATURES + ['won_toss']
    ...
    print(f"\n{'='*60}")
    print("RESULTS WITH won_toss FEATURE")
    print(f"{'='*60}")
    ...
    baseline_regret = 295.4
    improvement = baseline_regret - summary.mean_total_regret
    pct = improvement / baseline_regret * 100

    print(f"\n{'='*60}")
    print("IMPROVEMENT VS BASELINE (this test run)")
    print(f"{'='*60}")
    print(f"  Baseline regret: {baseline_regret:.1f}")
    print(f"  New regret: {summary.mean_total_regret:.1f}")
    print(f"  Improvement: {improvement:.1f} pts ({pct:.1f}%)")

    # Compare with original optimized (before feature engineering)
    original_regret = 286.0  # From earlier run with improved credits
    improvement_vs_original = original_regret - summary.mean_total_regret
    pct_vs_original = improvement_vs_original / original_regret * 100

    print(f"\n{'='*60}")
    print("IMPROVEMENT VS PREVIOUS BEST (with improved credits)")
    print(f"{'='*60}")
    print(f"  Previous best regret: {original_regret:.1f}")
    print(f"  New regret: {summary.mean_total_regret:.1f}")
    print(f"  Additional improvement: {improvement_vs_original:.1f} pts ({pct_vs_original:.1f}%)")

    if save_model:
        output_dir = Path("tmp/quantile_models_v2")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test best single feature")
    parser.add_argument(
        "--features",
    ...
    test_won_toss_feature(args.features, args.n_matches)
